-- JitlerUI.lua v5 — Premium Exploit Hub UI Library
-- Hex image tabs, shadow depth, compound toggle+keybind rows, rounder corners
...
local C = {
    ...
    Border    = Color3.fromRGB(32, 34, 48),
    SliderBg  = Color3.fromRGB(20, 22, 34),
    Green     = Color3.fromRGB(56, 198, 116),
    Red       = Color3.fromRGB(238, 56, 56),
    Yellow    = Color3.fromRGB(248, 198, 48),
    WidgetBorder = Color3.fromRGB(28, 30, 44),
    TabBg     = Color3.fromRGB(22, 22, 32),
...
function Library:CreateWindow(cfg)
    ...
    if hasLoading then
        loadScreen = mk("Frame", {Name="Loading", BackgroundColor3=C.BgOuter, Size=UDim2.new(1,0,1,0), ZIndex=100, BorderSizePixel=0, Parent=sg})
    ...
    -- ==================== GLASS SIDEBAR ====================
    local sidebar = mk("Frame", {Name="Sidebar", BackgroundColor3=C.Sidebar, BackgroundTransparency=0.25, Size=UDim2.new(0,SIDEBAR_W,1,0), BorderSizePixel=0, ClipsDescendants=false, Parent=main})
    -- Right edge line
    ...
    local brandGlow = mk("Frame", {BackgroundColor3=C.Accent, BackgroundTransparency=0.65, Size=UDim2.fromOffset(20,20), Position=UDim2.fromOffset(17,12), BorderSizePixel=0, Parent=sidebar}); rc(brandGlow,10)
    mk("Frame", {BackgroundColor3=C.Accent, Size=UDim2.fromOffset(6,6), Position=UDim2.fromOffset(24,19), BorderSizePixel=0, Parent=sidebar}); rc(brandGlow,3)
    ...
    -- Hex tab positioning: 50% out left, 40% inside bar, 10% out right
    local HEX_IMG = "rbxassetid://14482391301"
    local HEX_SIZE = 42
    local HEX_START_Y = 52
    local HEX_SPACING = 52
    local function getHexPos(idx)
        -- zig-zag: even tabs offset slightly right, odd offset left
        local baseX = -math.floor(HEX_SIZE * 0.5) -- 50% sticks out left
        local zigzag = (idx % 2 == 0) and 4 or -2
        local x = baseX + zigzag
    ...
    local function attachWidgets(target, wContent)
        ...
        function target:CreateToggleWithKeybind(tcfg, kcfg)
            ...
            UIS.InputBegan:Connect(function(input,gp)
                if listening then
                    if input.UserInputType==Enum.UserInputType.Keyboard then
                        currentKey=input.KeyCode.Name; keyBtn.Text="["..currentKey.."]"; listening=false
                        tw(keyBtn,{BackgroundColor3=C.Bg},0.1); DebouncedSave()
                    end
                    return
                end
                if not gp and input.KeyCode~=Enum.KeyCode.Unknown and input.KeyCode.Name==currentKey then
            ...
            if tcfg.Flag then Flags[tcfg.Flag]={get=function() return value end, set=function(v) toggle:Set(v==true) end} end
            if kcfg and kcfg.Flag then Flags[kcfg.Flag]={get=function() return currentKey end, set=function(v) if type(v)=="string" and #v>0 then currentKey=v; keyBtn.Text="["..v.."]" end end} end
            return toggle
        end
    ...
    function Window:CreateTab(tabArg)
        ...
        local hexBtn = mk("ImageButton", {
            ...
            Size = UDim2.fromOffset(HEX_SIZE, HEX_SIZE),
        ...
        local hexGlow = mk("ImageLabel", {
            ...
            BackgroundTransparency = 1,
            Size = UDim2.fromOffset(HEX_SIZE+10, HEX_SIZE+10),
            Position = UDim2.fromOffset(hx-5, hy-5),
            ScaleType = Enum.ScaleType.Stretch,
            ZIndex = 0,
            Parent = sidebar,
        })
        ...
        if tabIcon and tabIcon ~= "" then
            iconImg = mk("ImageLabel", {Image=tabIcon, ImageColor3=isFirst and C.Text or C.Dim, BackgroundTransparency=1, Size=UDim2.fromOffset(18,18), Position=UDim2.new(0.5,-9,0.5,-9), ScaleType=Enum.ScaleType.Fit, ZIndex=2, Parent=hexBtn})
        else
            iconTxt = mk("TextLabel", {Text="\226\154\153", TextColor3=isFirst and C.Text or C.Dim, Font=F.Bold, TextSize=16, BackgroundTransparency=1, Size=UDim2.new(1,0,1,0), ZIndex=2, Parent=hexBtn})
        end
        ...
        hexBtn.MouseEnter:Connect(function() if activeTab~=tabName then tw(hexBtn,{ImageColor3=C.AccentH},0.1) end end)
        hexBtn.MouseLeave:Connect(function() if activeTab~=tabName then tw(hexBtn,{ImageColor3=C.Dim},0.1) end end)
